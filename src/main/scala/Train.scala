...
//   }
  ...
  def main(args: Array[String]) {
    ...
    .collect() 
    .toMap
    val transformMatrixs = coreEmbeddings.map(x => (x._1, (inv(x._2.t * x._2) * x._2.t * coreEmbeddings(anchorIndex))))
    println("transformMatrixs", transformMatrixs.size)
    transformMatrixs.map(x => x._1).foreach(println)
    ...
    .map(x => {
        if (x._1 == anchorIndex)
            ...
            val newEmbeddingMatrix = (oldEmbedding * transformMatrix).t
